...
def parse_args():
    ...
    parser.add_argument('--eot_step', type=int, default=100, help='EOT sampling steps')
...
# point_cloud: (b, num, c)
def random_rot_3axis(point_cloud):
    batch_size = point_cloud.size(0)
    rotations = np.random.uniform(-np.pi, np.pi, (batch_size,3))

    rotations = torch.from_numpy(rotations).float().cuda().detach()
    ...
    return rotated_pc.transpose(2, 1)
...
def FGSM(classifier, criterion, points, targets, args):
    ...
    loss = 0
    # points: (b=1, c, num)
    for i in range(args.eot_step):
        # points: 
        points_rot = random_rot_3axis(points.transpose(2, 1))
        pred, _ = classifier(points_rot)
        loss += criterion(pred, targets)
