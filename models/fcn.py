...
class FCN32s(nn.Module):
    ...
    def __init__(self, nclass, backbone='vgg16', normal_channel=True, aux=False, pretrained_base=True,
                 norm_layer=nn.BatchNorm2d, **kwargs):
        super(FCN32s, self).__init__()
        self.aux = aux
        if backbone == 'vgg16':
            if normal_channel:
                self.pretrained = vgg16_seg(nclass, 3).features
            else:
                self.pretrained = vgg16_seg(nclass, 1).features
...
class FCN32s_part(nn.Module):
    """There are some difference from original fcn"""

    def __init__(self, npart, nclass=16, backbone='vgg16', normal_channel=True, aux=False, pretrained_base=True,
                 norm_layer=nn.BatchNorm2d, **kwargs):
        super(FCN32s_part, self).__init__()
        self.aux = aux
        self.n_cls = nclass
        if backbone == 'vgg16':
            if normal_channel:
                self.pretrained = vgg16_seg(npart, 3).features
            else:
                self.pretrained = vgg16_seg(npart, 1).features
            # self.pretrained = vgg16_seg(pretrained=pretrained_base).features
        else:
            raise RuntimeError('unknown backbone: {}'.format(backbone))
        self.head = _FCNHead(512+self.n_cls, npart, norm_layer)
        # self.head = _FCNHead(512, npart, norm_layer)
        if aux:
            self.auxlayer = _FCNHead(512+self.n_cls, npart, norm_layer)
            # self.auxlayer = _FCNHead(512, npart, norm_layer)
    ...
    def forward(self, x, label):
        ...
        pool5 = torch.cat([pool5, label.squeeze(1)[:, :, None, None].repeat(1, 1, H, W)],1)
