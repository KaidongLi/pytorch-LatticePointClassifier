...
from prompted_ViT import PromptedVisionTransformer
...
class get_model(nn.Module):
    ...
    def __init__(self, k=40, normal_channel=True, backbone='vit-l14', s=224*3):
        ...
        dim_feat = 768
        if backbone == 'vit-l14':
            self.backbone_2d_foundation = PromptedVisionTransformer(
                                    num_prompt=50,
                                    input_resolution=s//3,
                                    patch_size=14,
                                    width=1024,
                                    layers=24,
                                    heads=16,
            ...
            self.backbone_2d_foundation.load_state_dict(
                        torch.load('checkpoints/foundation_clip_ViT-L14.pth'), 
                        strict = False,
                        # map_location ='cpu'
                    )

            for key, param in self.backbone_2d_foundation.named_parameters():
                if 'prompt' in key:
    ...
    def forward(self, x):
        ...
        # network takes [b, c, size, size]
        outputs = self.backbone_2d_foundation(splatted_2d)
