...
PEND_ORIG = 'orig'
# PEND_NEW = 'splat'
PEND_IND_ORI = 'cluster_orig'
PEND_ATT = 'cluster'
PEND_ATT_FAIL = 'cluster_f'
PEND_ATT2D = '2dimg'
PEND_PRED = 'pred'
PEND_DIST = 'dists'
...
def evaluate():
    ...
    for file in os.listdir(args.img_dir):
        if file.endswith(PEND_ORIG + '.npy') and not file.endswith(PEND_IND_ORI + '.npy'):
            ...
            pt_cld_dist= np.load( os.path.join(args.img_dir, file.split(PEND_ORIG)[0]+PEND_DIST+'.npy') )
            ...
            for i in range(pt_cld_ori.shape[0]):
                ...
                and (pt_cld_atts[i].max() - pt_cld_atts[i].min() > 0) :
                    flg = True
                    norm_succ.append(1)
                    np_att_pc = np.concatenate((np.concatenate(pt_cld_atts[i], axis=0), pt_cld_ori[i]), axis=0)
                    ...
                    ct_succ[vic][tgt] += 1
                    l_fd.append(pt_cld_dist[None, i])
                ...
                and (pt_cld_atts[i].max()==1 and pt_cld_atts[i].min()==1) :
                    flg = False
                    norm_fail.append(1)
                    np_att_pc = np.concatenate((np.concatenate(pt_cld_attf[i], axis=0), pt_cld_ori[i]), axis=0)
    ...
    l_fd = np.concatenate(l_fd, axis=0)  # [num_data, K, 3]
    print('mean fd: ', l_fd.mean())

    print('Continue to show class-wise rate!! Continue???.....c' )
    # pdb.set_trace()
    ...
    np.savez(os.path.join(args.img_dir, 'pntnet_cluster_6.npz'),
