...
from models.DUP_Net import DUPNet
...
def attack_one_batch(classifier_0, classifier, criterion, points_ori, attacked_label, args, optimizer=None):
    ...
    for out_step in range(BINARY_SEARCH_STEP):
        ...
        for iteration in range(NUM_ITERATIONS):
            ...
            points_cls, attacked_label = points.cuda(), attacked_label.cuda()
            ...
            pts_dup = classifier_0(points_cls)
            pts_dup = pts_dup[:, :NUM_POINT].transpose(2, 1)
            pred, _ = classifier(pts_dup)
...
def main(args):
    ...
    atk_dir = experiment_dir.joinpath('attacked_dup%s/' % (args.file_affix))
    ...
    classifier_0 = DUPNet(sor_k=2, sor_alpha=1.1, npoint=1024, up_ratio=4)
    classifier_0 = classifier_0.cuda()
    classifier_0.pu_net.load_state_dict( torch.load('dump/pu-in_1024-up_4.pth') )
    ...
    for victim in CLASS_ATTACK:
        ...
        for j in range(args.num_sample//args.batch_size):
            ...
            # images: b * num_pts * c
            dist, img, preds, img_2d, img_f = attack_one_batch(classifier_0, classifier, criterion, images, targets, args)
...
if __name__ == '__main__':
    ...
    logger.setLevel(logging.INFO)
    formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
    file_handler = logging.FileHandler('%s/%s_pert_dup%s.txt' % (log_dir, args.model, args.file_affix))
    file_handler.setLevel(logging.INFO)
    file_handler.setFormatter(formatter)
    logger.addHandler(file_handler)
